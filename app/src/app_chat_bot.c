...
{
    switch(event->type) {
        /* Recognized user speech — feed the Kaleidoscope shell (guardian +
         * active facet). This is the live ASR hook on the T5AI/ai_ui path. */
        case AI_USER_EVT_ASR_OK: {
            AI_NOTIFY_TEXT_T *txt = (AI_NOTIFY_TEXT_T *)event->data;
            if (txt && txt->data) {
                kaleido_on_voice(txt->data);
            }
        } break;

        /* Reactive BMO face: mouth animates while speaking, calm when done. */
        case AI_USER_EVT_TTS_START:
            kaleido_face_set("SPEAKING");
            break;
        case AI_USER_EVT_TTS_STOP:
        case AI_USER_EVT_TTS_ABORT:
            kaleido_face_set("STANDBY");
            break;
...
OPERATE_RET app_chat_bot_init(void)
{
    ...
    AI_CHAT_MODE_CFG_T ai_chat_cfg = {
        /* Push-to-talk: press & hold the KEY button, speak, release. (The built-in
         * "Hi Tuya" wake word is weak on this chip model; a custom "BMO" wake word
         * needs a custom Tuya KWS model. AI_CHAT_MODE_WAKEUP / _FREE are the
         * hands-free alternatives once a good wake word is available.) */
    ...
    // Register the two-board Brain facet (OrangePi/Armada status over Wi-Fi)
    // before the shell starts and reads the facet list.
    kaleido_register(&kaleido_app_brain);

    // Trader facet: live PandaAI trading signals from the same OrangePi.
    kaleido_register(&kaleido_app_trader);

    // Start the brain link right away: the Armada daemon drives the body
    // (face/toast commands) even before anyone opens the Brain facet.
    extern void brain_facet_start(void);
    brain_facet_start();

    // Bring up the Kaleidoscope shell (intro -> BMO home -> apps). The ai_ui
    // chat screen now becomes the "Chat" facet inside the launcher.
    kaleido_start();
#endif
