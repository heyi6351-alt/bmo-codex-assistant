...
/* Companion add-ons: BMO-style animated face + distress guardian. */
#include "app_face.h"
...
static __attribute__((unused)) void __app_display_msg_handle(DISPLAY_MSG_T *msg_data)
{
    ...
    case TY_DISPLAY_TP_EMOTION: {
        ...
        app_face_set_by_name(msg_data->data); // drive the BMO face
    } break;
    case TY_DISPLAY_TP_STATUS: {
        // STANDBY / LISTENING / SPEAKING
    ...
    case TY_DISPLAY_TP_USER_MSG: {
        ...
        app_guardian_on_text(msg_data->data); // listen for distress
    } break;
...
static void __chat_bot_ui_task(void *args)
{
    ...
    ui_init();
    app_face_init(lv_scr_act()); // build the BMO face into the active screen
...
OPERATE_RET app_display_init(void)
{
    ...
    app_guardian_init(); // start the distress watcher
